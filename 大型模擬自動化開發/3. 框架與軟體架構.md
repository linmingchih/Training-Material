...
- **任務排程器 (Scheduler)**: 將使用者請求轉成可追蹤的作業，統一管理執行順序、狀態與結果回寫。
- **數位資產管理**: 自動化提取數據、產出報告，並將結果結構化儲存。
- **可擴展服務化**: 將核心模擬能力（例如 PyAEDT / PyEDB）封裝為可被前端與流程系統重複呼叫的服務。
...
graph TD
    ...
    subgraph Backend["後端核心 (Python Engine - 非同步架構)"]
        ...
        Manager[Scheduler / Simulation Manager]
        PyAEDT[PyAEDT / PyEDB]
        Pandas[Pandas / skrf]
...
### 動態訊息輸出機制 (Real-time Logging / SSE)
...
### 任務排程器 (Scheduler)
Scheduler 負責把模擬任務排入佇列、分配執行時機，並在任務完成後更新作業狀態與結果索引。對單機自動化場景而言，它可以是記憶體內的工作佇列，也可以是以時間戳目錄與狀態檔案為中心的輕量排程層；重點是讓長時間模擬與 UI 操作分離，避免使用者直接等待底層執行細節。

### PyAEDT / PyEDB
PyAEDT 與 PyEDB 是這套系統的核心工程自動化介面。前者負責驅動 Ansys AEDT 類工具進行建模、求解與結果擷取，後者則偏向 PCB / Package 資料結構與版圖操作。將兩者包裝在獨立模組中，可以讓模擬腳本、資料分析與前端流程保持清楚分工。
